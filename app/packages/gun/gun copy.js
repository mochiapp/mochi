;(function () {
  ...
  ;(function (Util) { // Generic javascript utilities.
    ;(function (Type) {
      ...
      Type.obj = {is: function (o) { return !o || !o.constructor ? false : o.constructor === Object ? true : !(!o.constructor.call || o.constructor.toString().match(/\[native\ code\]/)) }}
  ...
  ;(function (Gun) { // Gun prototype chain methods.
    ...
    Gun.chain.put = function (val, cb, opt) {
      ...
      function put (at) {
        ...
        function end (err, ify) {
          ...
          if (err = Gun.union(chain, ify.graph, {end: false,
            soul: function (soul) {
    ...
    Gun.chain.get = (function () {
      ...
      Gun.on('get').event(function (gun, at, ctx) {
        if (ctx.halt) { ctx.halt = false } // TODO: CLEAN UP with event emitter option?
      }, Infinity)
      return function (lex, cb, opt) { // get opens up a reference to a node and loads it.
        ...
        function load (lex) {
          ...
          if (ctx.force) { ctx.force = false } else if (cached) { return false }
    ...
    Gun.chain.key = (function () {
      ...
      return function (key, cb, opt) {
        ...
        function index (at) {
          ...
          ctx.obj = (Gun.is.node.soul(ctx.node, 'key') === 1) ? Gun.obj.copy(ctx.node) : Gun.obj.put({}, at.soul, Gun.is.rel.ify(at.soul))
      ...
      return function (path, cb, opt) {
        ...
        if (!Gun.list.is(path)) {
          if (!Gun.text.is(path)) {
            ...
            } else { return this.path(path + '', cb, opt) }
          } else { return this.path(path.split('.'), cb, opt) }
        ...
        gun._.at('path:' + path[0]).event(function (at) {
          ...
          if (path.length === 1) { cb.call(ctx.by.chain || chain, null, at.value, ctx.field) }
...
;(function (Tab) {
  ...
  Gun.on('opt').event(function (gun, opt) {
    ...
    tab.server = tab.server || function (req, res) {
      ...
      if (Gun.is.lex(req.body)) { return tab.server.get(req, res) } else { return tab.server.put(req, res) }
    }
  ...
  var request = (function () {
    ...
    r.ws = function (opt, cb) {
      ...
      ws.onclose = window.onbeforeunload = function (c) {
        ...
        if (c.code === 1006) { // websockets cannot be used
