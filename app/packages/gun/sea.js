;(function () {
  ...
  USE(function (module) {
    ...
    SEA.verify = async (data, pair, cb) => {
      try {
        const json = parse(data)
        if (pair === false) { // don't verify!
          const raw = (json !== data)
  ...
  USE(function (module) {
    ...
    SEA.encrypt = async (data, pair, cb) => {
      try {
  ...
  USE(function (module) {
    ...
    SEA.decrypt = async (data, pair, cb) => {
      try {
        const key = pair.epriv || pair
        const json = parse(data)
    ...
    const queryGunAliases = (alias, gunRoot) => new Promise((resolve, reject) => {
      ...
      gunRoot.get(`alias/${alias}`).get((rat, rev) => {
        ...
        Gun.obj.map(rat.put, (at, pub) => {
          if (!pub.slice || pub.slice(0, 4) !== 'pub/') {
        ...
        each.any = function (val, key, node, soul, user) {
          ...
          if (!user || !(user = user._) || !(user = user.sea)) {
            if ((tmp = soul.split('~')) && tmp.length == 2) {
              ...
              SEA.verify(val, (pub = tmp[1]), function (data) {
                var rel
          ...
          if (!(tmp = soul.split('~')) || tmp.length !== 2) {
            ...
            if (val && val.slice && (val).slice(0, 4) === 'SEA{') {
              check['any' + soul + key] = 0
              each.end({ok: 1})
              return
            }
