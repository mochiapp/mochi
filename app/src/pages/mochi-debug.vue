      ...
      if (input.files && input.files[0]) {
        ...
        reader.onload = async function (e) {
          ...
          img.onload = async function () {
            ...
            let blob = await getBlob(rotatedCanvas)
            // let blob = await compressWithGuetzli(rotatedCanvas)

            console.log('img size', rotatedCanvas.width, rotatedCanvas.height)
            console.log('resized to canvas & created blob!', blob)

            that.$refs.test.src = blobToUrl(blob)

            that.magnetURIStored = await webTorrent.testStore(blob)
          }
        }
