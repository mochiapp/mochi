...
Gun.on('opt', function (ctx) {
  ...
  var opt = ctx.opt
  if (ctx.once) { return }
  if (opt.localStorage !== false && !process.env.AWS_S3_BUCKET) { return } // TODO: Remove this after migration.
  if (opt.radisk === false) { return }
  ...
  ctx.on('get', function (at) {
    ...
    rad(tmp, function (err, val) {
      ...
      ctx.on('in', {'@': id, put: Gun.graph.node(node), err: err || u})
    })
